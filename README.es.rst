...
Parámetro ``real_asyncio``
~~~~~~~~~~~~~~~~~~~~~~~~~~

FreezeGun tiene un parámetro adicional, ``real_asyncio``, que permite que los bucles de eventos de asyncio vean el tiempo monótono real aunque ``time.monotonic()`` esté congelado. Esto es útil para evitar que ``asyncio.sleep()`` y otras funciones de asyncio que dependen del tiempo monótono dejen de funcionar correctamente.
...
A continuación se presenta una documentación concisa de la API con todas las opciones disponibles:
